# Lab 06 Orchestration Script
$ErrorActionPreference = "Stop"

Write-Host "Initializing Lab 06..."
go mod tidy

# Build first
./build.ps1

Write-Host "Starting Lab 06 processes (output streamed to this console)..."
...
function Run-Lab {
	$pBroker = $null; $pWorkers = @(); $pClient = $null
	$longRunning = @()
	try {
		$pBroker = Start-ChildProcess "scanner_broker.exe" "Broker"
		$longRunning += $pBroker
		Start-Sleep -Seconds 1

		for ($i=1; $i -le 3; $i++) {
			$w = Start-ChildProcess "av_engine.exe" "AV-$i"
			$pWorkers += $w
			$longRunning += $w
		}
		...
		Write-Host "Cluster active. Launching Upload Service..."
		$pClient = Start-ChildProcess "upload_service.exe" "Uploader"
		
		# Register Ctrl+C
		$cancelDel = Register-CtrlCHandler -procs ($longRunning + $pClient)
		[console]::add_CancelKeyPress($cancelDel)

		Write-Host "Lab 06 running. Press Ctrl+C to stop."

		while (-not $pBroker.HasExited) { 
			Start-Sleep -Milliseconds 200 
		}

		Write-Host "Broker exited. Cleaning up..."
	}
	catch {
		Write-Host "Error: $($_.Exception.Message)"
	}
	finally {
		Stop-Processes ($longRunning + $pClient)
		if ($cancelDel) { [console]::remove_CancelKeyPress($cancelDel) }
		Write-Host "All processes stopped."
		Write-Host "Exiting Lab 06."
	}
}
